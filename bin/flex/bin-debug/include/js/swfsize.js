/*
	...
	// now all you need to do is create a new isntace of the SWFSize and setup its id;
...
this.onWindowLoadHandler = function(e)
{
	if(this.available())
	{
		this.updateMetrics(); this.targetSWF().onWindowInit(this.tY+","+this.bY+","+this.lX+","+this.rX+","+this.wW+","+this.wH+","+this.swfW+","+this.swfH+","+this.scrX+","+this.scrY);
	}else{
		this.errorID();
	}
}
...
this.setSWFWidth = function(value, valueType)
{
	...
	if(this.available())
	{
		...
		SWFSizePool.onWindowResizeHandler();
	}else{
		this.errorID();
	}
}
